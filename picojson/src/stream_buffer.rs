...
impl<'a> StreamBuffer<'a> {
    ...
    /// Compact buffer by moving unprocessed data to start
    /// This makes space for new data when buffer is full
    ///
    /// Returns the offset by which data was moved (for updating parser state positions)
    pub fn compact(&mut self) -> Result<usize, StreamBufferError> {
        if self.tokenize_pos == 0 {
            // Already at start, no compaction needed
            return Ok(0);
        }

        let offset = self.tokenize_pos;
        ...
        let remaining_data = self.data_end - self.tokenize_pos;
        self.buffer.copy_within(self.tokenize_pos..self.data_end, 0);
