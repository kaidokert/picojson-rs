...
impl<R: Reader, C: BitStackConfig> StreamParser<'_, R, C> {
    ...
    fn next_event_impl(&mut self) -> Result<Event<'_, '_>, ParseError> {
        ...
        loop {
            ...
            while !self.have_events() {
                ...
                if self.stream_buffer.is_empty() {
                    log::debug!(
                        "StreamBuffer is empty, processing state: {:?}",
                    ...
                    log::debug!(
                        "StreamBuffer is not empty, processing state: {:?}",
                        self.processing_state
                    );
                    ...
                    log::debug!("Tokenizing byte: {:?} ('{}')", byte, byte as char);
    ...
    fn extract_number_from_state_with_context(
        ...
        log::debug!(
            "Extracting number: token_start={}, from_container_end={}",
            number_token_start,
            from_container_end
        );
    ...
    fn fill_buffer_from_reader(&mut self) -> Result<(), ParseError> {
        // First attempt: try to fill existing space
        let data_end = self.stream_buffer.data_end();
        let buffer_len = self.stream_buffer.buffer_len();

        if let Some(fill_slice) = self.stream_buffer.get_fill_slice() {
            log::debug!(
                "Buffer has space: {} bytes available, data_end: {}, buffer_len: {}",
                fill_slice.len(),
                data_end,
                buffer_len
            );
            ...
            log::debug!("Reader provided {} bytes", bytes_read);
            ...
            let compact_start_pos = match self.parser_state.state {
                crate::shared::State::Number(start_pos) => {
                    log::debug!(
                        "Buffer full, in Number state, compacting from: {}",
                        start_pos
                    );
                    start_pos
                }
                crate::shared::State::Key(start_pos) => {
                    log::debug!("Buffer full, in Key state, compacting from: {}", start_pos);
                    start_pos
                }
                crate::shared::State::String(start_pos) => {
                    log::debug!(
                        "Buffer full, in String state, compacting from: {}",
                        start_pos
                    );
                    start_pos
                }
                _ => {
                    ...
                    log::debug!("Buffer full, not in token, compacting from: {}", pos);
                    pos
                }
            };
            ...
            log::debug!(
                "Compaction completed. Offset: {}, New tokenizer pos: {}, New data end: {}",
                offset,
                self.stream_buffer.current_position(),
                self.stream_buffer.data_end()
            );
            ...
            log::debug!("Updating parser positions after compaction");
            ...
            if let Some(fill_slice) = self.stream_buffer.get_fill_slice() {
                ...
                log::debug!("Filled {} bytes after compaction", bytes_read);
                self.stream_buffer.mark_filled(bytes_read)?;
            } else {
                log::debug!("No space available even after compaction");
            }
        }
        Ok(())
    }
    ...
    fn update_positions_after_compaction(&mut self, offset: usize) -> Result<(), ParseError> {
        log::debug!(
            "Before position update: state={:?}, offset={}",
        ...
        log::debug!("After position update: state={:?}", self.parser_state.state);
        Ok(())
    }
    ...
    fn switch_key_to_escape_mode(
        ...
        log::debug!(
            "Switching key to escape mode: original_pos={}, offset={}",
            original_pos,
            offset
        );
        ...
        log::debug!("Discarded key content length: {}", discarded_content);
        ...
        log::debug!("No content discarded, continuing key parsing");
        Ok(())
    }
    ...
    fn switch_string_to_escape_mode(
        ...
        log::debug!(
            "Switching string to escape mode: original_pos={}, offset={}",
            original_pos,
            offset
        );
        ...
        log::debug!("Discarded string content length: {}", discarded_content);

        if discarded_content > 0 {
            // We lost some actual string content - this would require content recovery
            // For now, this is unsupported
            log::debug!("String content was discarded - unsupported case");
            return Err(ParseError::ScratchBufferFull);
        }

        // No actual content was discarded, we can continue parsing
        // We can continue parsing the string from the current position
        log::debug!("No content discarded, continuing string parsing");
        Ok(())
    }
