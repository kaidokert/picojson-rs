...
fn should_succeed(buffer_size: usize) -> bool {
    // EMPIRICAL: Testing reveals the actual threshold through experimentation
    // The minimum working buffer size depends on chunk size - with large chunks,
    // we need more buffer space to avoid content discarding during compaction
    // TODO: Update this threshold based on actual test results
    buffer_size >= 9 // Empirically determined: buffer size 9 is the minimum that works
}
