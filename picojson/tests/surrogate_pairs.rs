...
fn test_pathological_cases() {
    // High surrogate at end of string (becomes literal text)
    let input1 = r#"["\uD801"]"#;
    let expected1 = [
        Event::StartArray,
        Event::String(JsonString::Unescaped("\\uD801")), // Literal text
        Event::EndArray,
        Event::EndDocument,
    ];

    test_fixture(create_slice_parser(input1), &expected1);
    test_fixture(create_stream_parser_full(input1), &expected1);
    test_fixture(create_stream_parser_chunked(input1, 3), &expected1);
}
